...
#10
select (first_name)as name ,department_id,salary,commission_pct from employees
where commission_pct is not null
order by salary desc;

#11
select last_name,job_title from employees left join jobs
...
#(2)1
select min_salary,max_salary,avg(salary)as avg,commission_pct from employees join jobs
on employees.job_id=jobs.job_id
group by salary;

#2
select department_name,salary,commission_pct from employees join departments
...
#4
select department_id,(salary),count(first_name) from employees
group by department_id;
...
#8
select count( first_name), first_name, department_id  from employees
group by department_id
having count(first_name) >1;

#9
select sum(salary),first_name,manager_id from employees
group by first_name
having manager_id is not null;


#10
select manager_id,count(first_name),department_id from employees
group by  manager_id;

#11
select first_name,last_name,department_id,salary,manager_id from employees
group by manager_id
having  last_name like "_a%";
...
#2
select concat(first_name,"-",last_name),date_format(hire_date,"%m") from employees;

#3
select last_name,salary,
case
    when salary>10000 then salary + (salary*0.1)
else salary +(salary*0.11) and salary+1500
end as appropriate
from employees;    

#4
select concat(employee_id,"00", "E") ,department_id,salary,manager_id from employees;


#5
select last_name as appropriate,char_length(last_name) from employees
where last_name like "J%" or last_name like "A%" or last_name like "M%";

#6
select concat(salary,"$")as SALARY ,last_name  from employees;
...
#10
select lower(concat(first_name, "@systechusa.com")) as EMAIL from employees;


#11
select job_title, first_name from employees join jobs
on employees.job_id=jobs.job_id
where first_name= "Trenna";


#12
select  first_name,department_id,city from emp_details_view
having first_name ="Trenna" ;
...
#2
select job_title,department_name,city from emp_details_view 
where department_id = 40 group by job_title;


#3
select last_name,department_name,location_id,city,commission_pct from emp_details_view
where commission_pct is not null;
...
#5
select last_name,department_id,department_name,job_title from emp_details_view
where city=" ATLANTA";
...
#8
 select FIRST_NAME, DEPARTMENT_ID from  emp_details_view
 where DEPARTMENT_ID 
 IN (select DEPARTMENT_ID from emp_details_view
 GROUP BY DEPARTMENT_ID 
 HAVING COUNT(*)<2);
...
#8
select concat(first_name," ",last_name) as emp_name from emp_details_view 
where job_title in (select job_title from emp_details_view where department_name = "Sales");
...
#11
select first_name,salary,ifnull(commission_pct,"0") from employees;
